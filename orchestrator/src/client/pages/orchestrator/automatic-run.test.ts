...
describe("automatic-run utilities", () => {
  ...
  it("assigns a dedicated Kalibrr max-jobs limit", () => {
    const limits = deriveExtractorLimits({
      budget: 120,
      searchTerms: ["backend", "platform"],
      sources: ["kalibrr"],
    ...
    expect(limits.kalibrrMaxJobsPerTerm).toBeGreaterThan(0);
  ...
  it("assigns a dedicated Tech in Asia max-jobs limit", () => {
    const limits = deriveExtractorLimits({
      budget: 120,
      searchTerms: ["backend", "platform"],
      sources: ["techinasia"],
    ...
    expect(limits.techinasiaMaxJobsPerTerm).toBeGreaterThan(0);
  ...
  it("includes kalibrr in estimate caps using the shared term budget", () => {
    const estimate = calculateAutomaticEstimate({
      values: {
        topN: 10,
        minSuitabilityScore: 50,
  ...
  it("includes techinasia in estimate caps using the shared term budget", () => {
    const estimate = calculateAutomaticEstimate({
      values: {
        topN: 10,
        minSuitabilityScore: 50,
